...
function setfields()
{
	var query="SELECT * FROM Pipeline";
	...
	x = document.getElementById("terror").selectedIndex;
	if(x!=0)
	{
		y = document.getElementById("terror").options;
		document.getElementById("ter1").value=y[x].text;
		var where = (used ? " AND" : " WHERE");
...
<br/> <br/>
    ...
    <div id="middle_box">
	  ...
	  <p> <b> Pipeline Search Criteria</b> </p>
	<br/>
       	<!---Dropdown Lists--> 

	<form action="searchPipeline.php" method="post">
	...
	$sql = mysql_query("SELECT DISTINCT abstraction FROM Pipeline WHERE abstraction!='' ;");
	...
	/* Select database to populate dropdown*/
	$db_selection= mysql_select_db($database, $connection);
	$sql = mysql_query("SELECT DISTINCT url  FROM Pipeline WHERE url!='' ;");
	...
	/* Select database to populate dropdown*/
	$db_selection= mysql_select_db($database, $connection);
	$sql = mysql_query("SELECT DISTINCT viewerset  FROM Pipeline  WHERE viewerset!='' ;");
	...
	/* Select database to populate dropdown*/
	$db_selection= mysql_select_db($database, $connection);
	$sql = mysql_query("SELECT DISTINCT sourceformat  FROM Pipeline  WHERE sourceformat!='' ;");
	...
	/* Select database to populate dropdown*/
	$db_selection= mysql_select_db($database, $connection);
	$sql = mysql_query("SELECT DISTINCT sourcetype  FROM Pipeline  WHERE sourcetype!='' ;");
	...
	/* Select database to populate dropdown*/
	$db_selection= mysql_select_db($database, $connection);
	$sql = mysql_query("SELECT DISTINCT targettype  FROM Pipeline  WHERE targettype!='' ;");
	...
	<input type="hidden" id="tf1" name="tf1" >
   	 <select name="tformat" id="tformat" style="width: 250px">
        	<?php 
	/* Select database to populate dropdown*/
	$sql = mysql_query("SELECT DISTINCT targetformat  FROM Pipeline  WHERE targetformat!='' ;");
	...
	<tr><td>
	<input type="hidden" id="ter1" name="ter1" >
   	 <select name="terror" id="terror" style="width: 250px">
        	<?php 
	/* Select database to populate dropdown*/
	$sql = mysql_query("SELECT DISTINCT error  FROM Pipeline  WHERE error!='' ;");
	echo "<option value=\"abs\">Errors and Successes</option>";
	while ($row = mysql_fetch_array($sql)){
...
<?php
	...
	if(isset($_POST['qry']))
	{
		...
		else
		{
			...
			while ($row = mysql_fetch_array($sql))
			{
				$imageDone = '<center><img src="' .$row['image']. '" height="100" width="150"></center>';
				...
				echo "<td>" . $imageDone . "</td>";
				echo "<td><form action='foundPipeline.php' method='post'><button type='submit'  class='btn btn-primary' name='id' id='viewd' value='" . $row['id'] . "' style='width:100px;'>View Details</button></form></td>";
				echo "</tr>";
